...
public class NativeAudioAssetComplex implements OnPreparedListener, OnCompletionListener {
	...
	public NativeAudioAssetComplex(AssetFileDescriptor afd, float volume, String fullPath)  throws IOException {
		state = INVALID;
		mp = new MediaPlayer();
		if (afd != null) {
			...
			if (fp.exists()) {
				...
				mp.setDataSource(Environment.getExternalStorageDirectory().getPath() + "/" + fullPath);
			}
		}
