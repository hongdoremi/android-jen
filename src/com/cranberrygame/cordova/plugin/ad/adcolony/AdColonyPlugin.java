...
public class AdColonyPlugin extends CordovaPlugin {
	...
	private void setUp(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
		...
		Log.d(LOG_TAG, String.format("%s", customId));
		...
		cordova.getActivity().runOnUiThread(new Runnable() {
			@Override
			public void run() {
				_setUp(appId, fullScreenAdZoneId, rewardedVideoAdZoneId, customId);
			}
		});
	}
	...
	private void _setUp(String appId, String fullScreenAdZoneId, String rewardedVideoAdZoneId, String customID) {
		this.appId = appId;
