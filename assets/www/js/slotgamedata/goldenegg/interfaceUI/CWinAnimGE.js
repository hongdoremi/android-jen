 ...
 * @constructor
 */
function CWinAnimGE(my, oParentContainer) {
    var _bUpdate;
    var _iSpeedStep;
    var _iScaleToReach;
    var _iScaleFactor;
    ...
    this._init = function (oParentContainer) {
        ...
        //my.currentGameSlot.s_oGame.playFreeSpinBackgroundMusic();

        _bUpdate = false;
        _iSpeedStep = 0;

        _oContainer = my.add.group();
        _oContainer.scale.setTo(settings.SCALE_SYMBOL_WIN_FROM);
        ...
        _oWinFrame = my.add.sprite(0,0, 'static_symbol', 'frame_win',_oContainer);
        _oWinFrame.x = _oContainer.x;
        _oWinFrame.y = _oContainer.y;
        _oWinFrame.anchor.setTo(0.5);
        _oWinFrame.scale.setTo(settings.SYMBOL_SCALE_NORMAL);
        //_oContainer.addChild(_oWinFrame);

        //ATTACH WIN BG AND TEXT
        _oWinTextBg = my.add.sprite(0,
            settings.SYMBOL_HEIGHT/2,
            "amount-win",
            null,
            _oContainer);
        _oWinTextBg.anchor.setTo(0.5);
        _oWinTextBg.scale.setTo(1);
        //_oWinTextBg.x += _oWinTextBg.width/6;
        _oWinText =  my.add.text(_oWinTextBg.position.x,_oWinTextBg.position.y, "x0", {
            font: "35px Skater-Girls-Rock",
     ...
     * @param bFreeSpin: boolean - true if this is in Free Spin state
     * @param bPlayOnce: boolean - true if play animation once (else play three times)
     */
    ...
    this.playAnimation = function (bFreeSpin, bPlayOnce, iRow, iCol) {
        ...
        for (var i = 0; i < my.slotSettings.s_iSymbolAnims[_iCurSymbol]; i++) {
            var oImage = my.add.sprite(0,0,"symbolGE" + _iCurSymbol + "_" + (i + 1));
            oImage.anchor.setTo(0.5);
            oImage.scale.setTo(settings.SYMBOL_SCALE_NORMAL);
            if (i === 0) {
                oImage.visible = true;
            } else {
        ...
        _oContainer.x += ((settings.SYMBOL_WIDTH) / 2) + 10;
        _oContainer.y += ((settings.SYMBOL_HEIGHT)/ 2) + 10;

        _oContainer.scale.setTo(settings.SCALE_SYMBOL_WIN_FROM);

        tweenAnimation = my.add.tween(_oContainer.scale).to( { x: settings.SCALE_SYMBOL_WIN_TO, y: settings.SCALE_SYMBOL_WIN_TO }, 1000, Phaser.Easing.Bounce.Out, true);
    ...
    this.reset = function () {
        ...
        //REMOVE ALL IMAGES
        var i = _aSymbolImages.length; while (i--) {_oImageAttach.removeChild(_aSymbolImages[i]); }
        //for (var i = 0; i < ; i++) {
        //
        //}

        //my.tween.remove(tweenAnimation);
        ...
        if (_iSpeedStep === settings.ANIM_SYMBOL_STEPS) {
            ...
            if (_iCurFrame === my.slotSettings.s_iSymbolAnims[_iCurSymbol] - 1) {
                ...
                if (_iNumCycles === _iTotAnimCycle) {
                    ...
                    _bUpdate = false;
                    tweenAnimation = my.add.tween(_oContainer.scale).to( { x: settings.SCALE_SYMBOL_WIN_FROM, y: settings.SCALE_SYMBOL_WIN_FROM }, 500 * _iMutiplyDelayForAnimWinFreeSpin, Phaser.Easing.Cubic.Out, true);
