 ...
 * @constructor
 */
function CGameGE(my, parentGroup, oData) {
    var _bUpdate = false;
    var _bAutoSpin;
    ...
    this._init = function () {
        ...
        _oBg = my.loadBG('game-bg', _oContainer,100/70);
        //_oBg.scale.setTo(100/70); //reduce 70% resolution

        _oBgFreeSpin = my.loadBG('bg-freespins', _oContainer,100/70);
        //if(ManagerForScale.is3x4resolution()){
        //    _oBg.x+=8;
        //    _oBgFreeSpin.x+=8;
        //}
        //_oBgFreeSpin = my.add.sprite(0, 0, 'bg-freespins', null, _oContainer);
        //_oBgFreeSpin.scale.setTo(100/70); //reduce 70% resolution
        ...
        _oWinAnim = new CWinAnimGE(my, _oAttachWinAnim);
        _oInterface = new CInterfaceGE(my, _oContainer);

        _oFooter = new CFooter(my, _oContainer, my.currentGameSlot.s_aMultiply , settings.NUM_PAYLINES, my.currentGameSlot.s_iMp);
        //_oFooter.setPosition(
        //    settings.CANVAS_WIDTH/2 - 550 + (ManagerForScale.is3x4resolution()?100:0), settings.CANVAS_HEIGHT - 150);


        //_oModel = my.add.sprite(195, 706, 'farmer', null, _oContainer);
        //_oModel.anchor.setTo(0.5);
        //_oModel.y += ManagerForScale.incrementHeight();

        _oFarmer = new SpriteAnimation(my,
            _oContainer,
            {x:195+ManagerForScale.offsetOutOfBounce_1920/2,y:706},
            {x:1,y:1},
            'farmer_',
            1,
            24,
        ...
        _oFarmer.playWithoutTimer();
        arrayOfAnimation.push(_oFarmer);

        _oLady = new SpriteAnimation(my,
            _oContainer,
            {x:_oFarmer.x,y:_oFarmer.y},
            {x:1,y:1},
            'lady_',
            1,
            24,
        ...
        _oDoubleUp = new CDoubleUpGE(my, parentGroup);

        _oBonusFreeSpin = new CBonusFreeSpinGE(my, parentGroup);
        if(my.currentGameSlot.readyToSpin) {
            this.initLines();
        }
        _oWinPanel = new CWinPanelGE(my, parentGroup);
        /////////////////////
        //Init state manager
        Manager4State.init();
...
//        if(my.currentGameSlot.finishAPI){
    ...
    this._initReels = function () {
        ...
        oReels.position = {x: 10, y: 5};
        //var bg = my.add.sprite(settings.CANVAS_WIDTH/2-8, settings.CANVAS_HEIGHT/2, 'reels', null, _oContainer);
        //var bg = my.add.sprite(settings.REEL_BG_OFFSET_X, settings.REEL_BG_OFFSET_Y, 'reels', null, _oContainer);
        //bg.anchor.setTo(0);
        ...
        for (var i = 0; i < settings.NUM_REELS; i++) {
            ...
            _aMovingColumns[i] = new CReelColumnGE(my, i, iXPos, iYPos, iCurDelay, aSymbols, _oAttachSymbols,
                _oAttachWinFrames);
            _aMovingColumns[i + settings.NUM_REELS] = new CReelColumnGE(my, i + settings.NUM_REELS, iXPos,
     ...
     * @param isToMainState: boolean - true if this sets to MainGame State
     */
    this.switchToMainState = function (isToMainState) {
        _oBgFreeSpin.visible = !isToMainState;
        //_oModelFreeSpin.visible = !isToMainState;
        //
        //_oModel.visible = isToMainState;
        if(isToMainState){
            _oFarmer.playWithoutTimer();
            _oLady.stop();
            _oLady.hide();
        }else{
            _oLady.playWithoutTimer();
            _oFarmer.stop();
            _oFarmer.hide();
        }
        _oBg.visible = isToMainState;
    };
    /**
        ...
        if (_iCurWinShown === _aWinningLine.length) {
            ...
            if (_iTotFreeSpin > 0) {
                ...
                if (_bShowTotalWinOnFreeSpin) {
                    ...
                    setTimeout(function () {
                        Manager4Sound.playBackgroundMusic();
                        my.currentGameSlot.s_oStage.getChildAt(2).visible = true;
                    }, 8000);
    ...
    this.generateFinalSymbols = function (aWheel, aWinPosition, aTableWin) {
        ...
        for (var k = 0; k < aWinPosition.length; k++) {
            ...
            //do {
            //    row = (iCont < aTmp.length) ? aTmp[iCont] - 1 : -1;
            //    col = (row > -1 && row < _aCurWheel.length && iCont < _aCurWheel[row].length) ? iCont : -1;
            //
            //    iValue = (col > -1) ? _aCurWheel[row][col] : undefined;
            //    //trace("_aCurWheel["+iCont+"]: "+_aCurWheel[iCont]);
            //    //iValue = _aCurWheel[aTmp[iCont]-1][iCont];
            //    iCont++;
            //} while ((iValue === undefined || (_iTotFreeSpin > 0 && iValue === settings.WILD_EXPANDED)) && iCont < settings.NUM_REELS);
            ...
            //while (_iTotFreeSpin > 0 && iValue === settings.WILD_EXPANDED) {
            //    iCol--;
            //    if (iCol < 0) {
            //        break;
            //    }
    ...
    this.showWinPanelAddFreeSpin = function () {
        ...
        _oInterface.stopWinTextAnimation();
        //setTimeout(function () {
        //    _oInterface.refreshFreeSpin(_iTotFreeSpin + 1, _iMultyFreeSpin);

        //var durationTxtAnimation = _oInterface.refreshBet(my.currentGameSlot.s_iCurWin, my.currentGameSlot.s_iTotBet);
        //}, 8000);
    ...
    this.onBonusFreeSpinStep1Received = function (iFinish, totalFreeSpins, totalMulty, aWheels) {
        ...
        if(LobbyConfig.isTestAlgorithmMode) {
            ...
            if(!iFinish) my.reloadProfileForTestSpin(true, my.getRandomParam(_oBonusFreeSpin.getArrayOfSelected()), true, settings.BONUS_TYPE_FREESPIN, this.getStepBonus() - 1, resultWin,"Free spin");
                ...
                _iStepBonus = 1;
                my.reloadProfileForTestSpin(true, 0, true, settings.BONUS_TYPE_FREESPIN, 2, resultWin,"Free spin");
            }
        }else{
