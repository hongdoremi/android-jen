 ...
 * @constructor
 */
function CWinAnimDB(my, oParentContainer) {
    var _bUpdate;
    var _iSpeedStep;
    var _iScaleToReach;
    var _iScaleFactor;
    ...
    this._init = function (oParentContainer) {
        ...
        _oWinTextBg = my.add.sprite(0,
            ...
            null,
            _oContainer);
        _oWinTextBg.anchor.setTo(1);
        _oWinTextBg.scale.setTo(1.5);
        _oWinTextBg.x += _oWinTextBg.width/6;
        _oWinText =  my.add.text(-_oWinTextBg.width/3.25,-_oWinTextBg.height/2, "x0", {
    ...
    this.playAnimation = function (bFreeSpin, bPlayOnce, iRow, iCol) {
        ...
        for (var i = 0; i < my.slotSettings.s_iSymbolAnims[_iCurSymbol]; i++) {
            var oImage = my.add.sprite(0,0,"symbolDB" + _iCurSymbol + "_" + (i + 1));
        ...
        _oContainer.x += Math.floor(settings.SYMBOL_WIDTH / 2) + 8;
        _oContainer.y += Math.floor(settings.SYMBOL_HEIGHT / 2) + 10;
