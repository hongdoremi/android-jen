 ...
 * @constructor
 */
function CReelColumnPO(my, iIndex, iXPos, iYPos, iDelay, aSymbols, oAttachSymbols, oAttachWinFrames) {
    var _oTextWinFrame;
    var that = new CReelColumnBase(my,
        ...
        oAttachWinFrames,
        null,
        my.gameSlotData.arrayPositionReelStop.easeOutElastic,
        function(_oAttachWinFrames, _oAttachSymbols){
            var _oSymbolHighlight = my.add.sprite(0, 0, 'static_symbol', 'symbol_0', _oAttachSymbols);
            _oSymbolHighlight.visible = false;
            _oTextWinFrame =  my.add.sprite(0, 0, 'static_symbol', 'symbol-wild-en', _oAttachSymbols);
            _oTextWinFrame.visible = false;
            //_oTextWinFrame.bringToTop();

            return {_oSymbolHighlight: _oSymbolHighlight, _oWinFrame : null};
        },
        1,
        1);
    /**
     //     * Initialize Frame Win animation
     //     */
    that.initFrameWin = function(){
        that._oWinFrame = new SpriteAnimation(my,
            that._oAttachWinFrames,
            {x:0,y:0},
            {x:1,y:1},
            'winFrame_',
            0,
    ...
    that.highlightSymbol = function (iIndex) {
        ...
        that._oSymbolHighlight.x = that._oContainer.x + that._aSymbols[iIndex].x;
        that._oSymbolHighlight.y = that._oContainer.y + that._aSymbols[iIndex].y;

        that._oSymbolHighlight.visible = true;

        that._oWinFrame.setPosition(that._oContainer.x + that._aSymbols[iIndex].x + 108,  that._oContainer.y + that._aSymbols[iIndex].y + 108);
        that._oWinFrame.play(25);

        switch (that._aSymbolValues[iIndex]){
            case settings.WILD_SYMBOL:
            {
                _oTextWinFrame.frameName = 'symbol-wild-en';
                break;
            }
            case settings.BONUS_SYMBOL:
            {
                _oTextWinFrame.frameName = 'symbol-bonus-en';
                break;
            }
            case settings.SCATTER_SYMBOL:
            {
                _oTextWinFrame.frameName = 'symbol-scatter-en';
                break;
            }
        }
        if(that._aSymbolValues[iIndex] >= 9){
            _oTextWinFrame.x = that._oContainer.x + that._aSymbols[iIndex].x;
            _oTextWinFrame.y = that._oContainer.y + that._aSymbols[iIndex].y;
            _oTextWinFrame.visible = true;
        }

        if (that._aSymbolValues[iIndex] === settings.SCATTER_SYMBOL) {
    ...
    that.showSymbol = null;
    /**
     * SHOW SYMBOL SPRITE ON ROW
     * @param iRow: row need to show
     */
    that.showSymbol = function (iRow) {
        that._aSymbols[iRow].visible = true;
        if (that._oSymbolHighlight.visible) {
            that._oWinFrame.stop();
            that._oWinFrame.hide();
            that._oSymbolHighlight.visible = false;
            _oTextWinFrame.visible = false;
        }
...
//
//        _oAttachWinSymbol = my.add.group();
//        _oAttachWinFrames.add(_oAttachWinSymbol);
//
//        this.initContainer(iXPos, iYPos, aSymbols);
//
//        _oSymbolHighlight = my.add.sprite(0, 0, 'static_symbol', 'symbol_0', _oAttachSymbols);
//        _oSymbolHighlight.visible = false;
//        _oTextWinFrame =  my.add.sprite(0, 0, 'static_symbol', 'symbol-wild-en', _oAttachSymbols);
//        _oTextWinFrame.visible = false;
//        //_oTextWinFrame.bringToTop();
...
//
//        _oWinFrame.setPosition(_oContainer.x + _aSymbols[iIndex].x + 108,  _oContainer.y + _aSymbols[iIndex].y + 108);
//        _oWinFrame.play(25);
//
//        switch (_aSymbolValues[iIndex]){
//            case settings.WILD_SYMBOL:
//            {
//                _oTextWinFrame.frameName = 'symbol-wild-en';
//                break;
...
//            }
//        }
//        if(_aSymbolValues[iIndex] >= 9){
//            _oTextWinFrame.x = _oContainer.x + _aSymbols[iIndex].x;
//            _oTextWinFrame.y = _oContainer.y + _aSymbols[iIndex].y;
//            _oTextWinFrame.visible = true;
//        }
//
...
//
//            arrayPositionStop = my.gameSlotData.arrayPositionReelStop.easeOutElastic.reel1;
//
//        }else{
//            _bContainsFinalSymbols = false;
//
//            arrayPositionStop = my.gameSlotData.arrayPositionReelStop.easeOutElastic.reel2;
//        }
//    };
//
//    /**
