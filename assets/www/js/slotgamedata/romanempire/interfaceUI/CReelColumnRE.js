 ...
 * @constructor
 */
function CReelColumnRE(my, iIndex, iXPos, iYPos, iDelay, aSymbols, oAttachSymbols, oAttachWinFrames) {
    var that = new CReelColumnBase(my,
        ...
        function(_oAttachWinFrames){
            ...
            var _oWinFrame = my.add.sprite(-5,-5, 'static_symbol', 'frame_win',_oAttachWinFrames);
    ...
    that.highlightSymbol = function (iIndex) {
        ...
        that._oWinFrame.x = that._oContainer.x + that._aSymbols[iIndex].x - 5;
        that._oWinFrame.y = that._oContainer.y + that._aSymbols[iIndex].y - 5;
        ...
        //    //_oSymbolHighlight.visible = false;
        //}else{
        that.hideSymbol(iIndex);
        //}

    };
    that.setSymbol = null;
    that.setSymbol = function (aSymbols) {
        var finalSymbol = aSymbols;
        var iCoutWild = 0;
        for (var i = 0; i < aSymbols.length; i++){
            if(aSymbols[i] == settings.WILD_SYMBOL){
                iCoutWild++;
            }
        }
        if(iCoutWild == 3){
            finalSymbol = aSymbols;
            var type = Math.random(0,0.9);
            if(type < 0.3){
                finalSymbol[0] = Math.floor(Math.random(0, 11));
                finalSymbol[1] = Math.floor(Math.random(0, 11));
            }else if(type < 0.6){
                finalSymbol[0] = Math.floor(Math.random(0, 11));
                finalSymbol[2] = Math.floor(Math.random(0, 11));
            }else{
                finalSymbol[1] = Math.floor(Math.random(0, 11));
                finalSymbol[2] = Math.floor(Math.random(0, 11));
            }
        ...
        //var iX = 0;
        //var iY = 0;
        for (var i = 0; i < finalSymbol.length; i++) {
            Lobby.PhaserJS.destroyAllChild(this._aSymbols[i]);
            that._aSymbols[i].frameName = 'symbol_' + finalSymbol[i];

            //oSymbolData.scale.setTo(2,2);
            //_aSymbols[i] = oSymbolData;

            that._aSymbolValues[i] = aSymbols[i];
...
//
//        _oWinFrame = my.add.sprite(-5,-5, 'static_symbol', 'frame_win',_oAttachWinFrames);
...
//        //var iY = 0;
//        for (var i = 0; i < finalSymbol.length; i++) {
//            _aSymbols[i].frameName = 'symbol_' + finalSymbol[i];
//
//            //oSymbolData.scale.setTo(2,2);
//            //_aSymbols[i] = oSymbolData;
//
...
//        //_oAttachWinFrames.addChild(_oSymbolHighlight);
//
//        _oWinFrame.x = _oContainer.x + _aSymbols[iIndex].x - 5;
//        _oWinFrame.y = _oContainer.y + _aSymbols[iIndex].y - 5;
