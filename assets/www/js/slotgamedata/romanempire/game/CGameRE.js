 ...
 * @constructor
 */
function CGameRE(my, parentGroup, oData) {
    var _bUpdate = false;
    var _bAutoSpin;
    ...
    this._init = function () {
        ...
        var oAvatar = my.add.sprite(200, settings.CANVAS_HEIGHT - 300, 'avatar', null, _oContainer);
        oAvatar.anchor.setTo(0.5);

        _oContainExpand = my.add.group();
        _oContainer.add(_oContainExpand);

        my.scaleCenterGroup(_oContainExpand);

        var createSpriteExpand = function(index){
            var x = settings.REEL_OFFSET_X + (settings.SYMBOL_WIDTH + settings.SPACE_BETWEEN_SYMBOLS)*(index) + settings.SYMBOL_WIDTH/2 + 8;
            var sprite = my.add.sprite(x, settings.REEL_OFFSET_Y + settings.HEIGHT_REEL/2 - 10, 'symbol-expand', null, _oContainExpand);
            sprite.anchor.setTo(0.5);
            sprite.visible = false;
            return sprite;
        };
        aSpriteExpand[0] = createSpriteExpand(1);
        aSpriteExpand[1] = createSpriteExpand(2);
        aSpriteExpand[2] = createSpriteExpand(3);
        //oAvatar.y = ManagerForScale.doubleIncrementHeight();
        ...
        _oWinAnim = new CWinAnimRE(my, _oAttachWinAnim);
        ...
        _oInterface = new CInterfaceRE(my, _oContainer);
        //
        //_oSettingsPanel = new CSettingsPanel();
        //
        //_oBonusJackPot = new CBonusJackPot(s_oAttachSection);
        _oBonusFreeSpin = new CBonusFreeSpinRE(my, parentGroup);
        _oDoubleUp = new CDoubleUpRE(my, parentGroup);
        /////////////////////
        _oWinPanel = new CWinPanelRE(my, parentGroup);
        //Init state manager
        Manager4State.init();
...
//                });
    ...
    this._initReels = function () {
        ...
        for (var i = 0; i < settings.NUM_REELS; i++) {
            ...
            _aMovingColumns[i] = new CReelColumnRE(my, i, iXPos, iYPos, iCurDelay, aSymbols, _oAttachSymbols,
                _oAttachWinFrames);
            _aMovingColumns[i + settings.NUM_REELS] = new CReelColumnRE(my, i + settings.NUM_REELS, iXPos,
        ...
        if (_iNumReelsStopped % 2 === 0) {
            ...
            for (var i = 0; i < settings.NUM_ROWS; i++) {
                ...
                if(symbol == settings.SCATTER_SYMBOL){
                    ...
                    break;
                }else if(symbol == settings.WILD_SYMBOL) {
                    shouldPlayWildStop = true;
                    break;
                }
            }

            if(shouldPlayWildStop){
                Manager4Sound.playWildStop();
                that.showExpand(_iNextColToStop);

            }else if (shouldPlayScatterStop) {
                Manager4Sound.playScatterStop(_iNextColToStop);
            } else {
     ...
     */
    this.showExpand = function(col) {
        if (aSpriteExpand[col - 1].visible){
            return;
        }
        aSpriteExpand[col - 1].alpha = 0;
        aSpriteExpand[col - 1].visible = true;
        my.add.tween(aSpriteExpand[col - 1]).to({alpha: 1}, 150, Phaser.Easing.Linear.None, true);
    };
    /**
     * Hide all Expand man
     */
    this.hideExpand = function(){
        _oContainExpand.alpha = 1;
        var tween = my.add.tween(_oContainExpand).to({alpha: 0}, 250, Phaser.Easing.Linear.None, true);
        tween.onComplete.add(function(){
            _oContainExpand.alpha = 1;
            for(var i = 0; i < aSpriteExpand.length; i++){
                aSpriteExpand[i].visible = false;
            }
        });
    };
    /**
     ...
     * @param col: column that need to check
     * @returns {boolean} true - the column can contains expand symbol
     */
    this.checkExpandColumn = function(col){
        return (_aFinalSymbolCombo[0][col] == settings.WILD_SYMBOL &&
        _aFinalSymbolCombo[1][col] == settings.WILD_SYMBOL &&
        _aFinalSymbolCombo[2][col] == settings.WILD_SYMBOL);
    };
    ...
    this.generateFinalSymbols = function (aWheel, aWinPosition, aTableWin) {
        ...
        for (var k = 0; k < aWinPosition.length; k++) {
            ...
            while(this.checkExpandColumn(col)){
                col--;
            }
     ...
     * @param totalMul: total multiplier
     * @param callback: callback after show
     */
    this.showFreeSpinBonusWinPanel = function(iWin, totalMul, callback){
        _oWinPanel.showBonusWinPanel(iWin, totalMul, callback);
    };
    /**
...
//dat fix loi ko du tien khi click vo play bonus ko duoc
    ...
    this.onBonusFreeSpinStep0Received = function (totalFreeSpins, totalMulty) {
        ...
        if(LobbyConfig.isTestAlgorithmMode) {
            _iStepBonus++;
            my.reloadProfileForTestSpin(true, 1,true, my.currentGameSlot.GameConstant.State.Bonus,this.getStepBonus()-1,null,"Spin Wheel");
        }else {
            _oBonusFreeSpin.show();
            _iStepBonus++;
        }
     ...
     * @param iTotWin: total win
     */
    this.onBonusFreeSpinStep1Received = function (iFinish, totalMulty, iTotWin) {
        if ( LobbyConfig.isDebug
            ) {
        ...
        if(LobbyConfig.isTestAlgorithmMode) {
            var resultWin = _oBonusFreeSpin.showResult(iFinish, totalMulty, iTotWin);
            my.reloadProfileForTestSpin(!iFinish, 1,true, my.currentGameSlot.GameConstant.State.Bonus,this.getStepBonus()-1,resultWin, "Spin Wheel");
        }else{
            _oBonusFreeSpin.showResult(iFinish, totalMulty, iTotWin);
        }
    ...
    this.onExit = function () {
        ...
        for(var i = 0; i < aSpriteExpand.length; i++){
            aSpriteExpand[i].destroy();
            aSpriteExpand[i]= null;
        }
        aSpriteExpand = null;
